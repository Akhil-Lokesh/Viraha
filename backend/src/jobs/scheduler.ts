 ...
 *
 * ENABLE_JOBS is read directly from process.env (not src/config/env.ts —
 * that file is owned by another agent in this wave). Accepted values:
...
export function jobsEnabled(): boolean {
  const explicit = parseEnvFlag(process.env.ENABLE_JOBS);
